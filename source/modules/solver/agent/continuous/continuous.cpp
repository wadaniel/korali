...
std::vector<float> Continuous::calculateKLDivergenceGradient(const policy_t &oldPolicy, const policy_t &curPolicy)
{
  ...
  if (_policyDistribution == "Clipped Normal")
  {
    for (size_t i = 0; i < _problem->_actionVectorSize; ++i)
    {
      ...
      // KL-Gradient with respect to Mean
      KLDivergenceGradients[i] = normalCDF(_actionLowerBounds[i], oldMu, oldSigma)*std::exp(normalLogDensity(_actionLowerBounds[i], curMu, curSigma) - normalLogCDF(_actionLowerBounds[i], curMu, curSigma));
      printf("f %f %f %f %f %f\n", normalCDF(_actionLowerBounds[i], oldMu, oldSigma), normalCDF(_actionLowerBounds[i], curMu, curSigma), std::exp(normalLogDensity(_actionLowerBounds[i], curMu, curSigma)), normalLogCDF(_actionLowerBounds[i], curMu, curSigma), normalLogDensity(_actionLowerBounds[i], curMu, curSigma));
      fflush(stdout);
      ...
      assert(isfinite(KLDivergenceGradients[i]));
      KLDivergenceGradients[i] -= normalCCDF(_actionUpperBounds[i], oldMu, oldSigma)*std::exp(normalLogDensity(_actionUpperBounds[i], curMu, curSigma)-normalLogCCDF(_actionUpperBounds[i], curMu, curSigma));
      printf("f %f %f %f %f %f\n", normalCDF(_actionUpperBounds[i], oldMu, oldSigma), normalCCDF(_actionUpperBounds[i], curMu, curSigma), std::exp(normalLogDensity(_actionUpperBounds[i], curMu, curSigma)), normalLogCCDF(_actionUpperBounds[i], curMu, curSigma), normalLogDensity(_actionUpperBounds[i], curMu, curSigma));
      fflush(stdout);
      assert(isfinite(KLDivergenceGradients[i]));

      // KL-Gradient with respect to Sigma
      KLDivergenceGradients[_problem->_actionVectorSize + i] = (_actionLowerBounds[i]-curMu)/curSigma*normalCDF(_actionLowerBounds[i], oldMu, oldSigma)/normalCDF(_actionLowerBounds[i], curMu, curSigma)*std::exp(normalLogDensity(_actionLowerBounds[i], curMu, curSigma));
