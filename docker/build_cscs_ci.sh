...
if [ "$#" -ne 1 ]; then
    ...
    echo "Building Docker image with $1 support on base image $(grep FROM Dockerfile_cscs_ci.build-env.$1 | cut -d ' ' -f2-)."
else
    echo "${ERROR_MSG}"
    exit 1
fi
...
IMAGE_BASENAME="${CONTAINER_REGISTRY}/korali_cscs_ci/$1"
...
BUILD_ENV_IMAGE=${BUILD_ENV_IMAGE:-"${IMAGE_BASENAME}/build-env"}
DEPLOY_IMAGE_UNTAGGED=${DEPLOY_IMAGE_UNTAGGED:-"${IMAGE_BASENAME}/deploy"}
DEPLOY_IMAGE=${DEPLOY_IMAGE:-"${DEPLOY_IMAGE_UNTAGGED}:${COMMIT_SHA}"}

set -x
if [[ -z $(docker images -q ${BUILD_ENV_IMAGE}) ]]; then
    docker build -f Dockerfile_cscs_ci.build-env.$1 -t ${BUILD_ENV_IMAGE} .
fi
if [[ -z $(docker images -q ${DEPLOY_IMAGE}) ]]; then
    docker build --build-arg BUILD_ENV=${BUILD_ENV_IMAGE} -f Dockerfile_cscs_ci.deploy.$1 -t ${DEPLOY_IMAGE} .
