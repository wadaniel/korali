...
void DeepSupervisor::initialize()
{
  ...
  // Check whether the minibatch size (N) can be divided by the requested concurrency
  if (_problem->_inferenceBatchSize % _batchConcurrency > 0) KORALI_LOG_ERROR("The inference concurrency requested (%lu) does not divide the training mini batch size (%lu) perfectly.", _batchConcurrency, _problem->_inferenceBatchSize);
  ...
  if (_batchConcurrency > 1) batchSizes.push_back(_problem->_inferenceBatchSize / _batchConcurrency);
...
void DeepSupervisor::runTestingGeneration()
{
 ...
 if (_problem->_inferenceBatchSize != _problem->_inputData.size())
   KORALI_LOG_ERROR("Inference Batch size %lu different than that of input data (%lu).\n", _problem->_inputData.size(), _problem->_inferenceBatchSize);
 ...
 if (_batchConcurrency > 1)
 {
  ...
  size_t NW = _problem->_inferenceBatchSize / _batchConcurrency;
