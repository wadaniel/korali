...
Simulation * initializeEnvironment(korali::Sample &s)
{
    ...
    argumentString = argumentString + obstacle;
...
double getReward(StefanFish *agent, const SimulationData & sim, const int agentID)
{
    ...
    static double dold = std::pow((initialPositions[agentID][0]-Xt)*(initialPositions[agentID][0]-Xt) 
                                + (initialPositions[agentID][1]-Yt)*(initialPositions[agentID][1]-Yt),0.5);

    const double retval = dold-d;
    dold = d;

    if (d < 0.05) return 100.0;
