...
void Conduit::runSample(Sample *sample, Engine *engine)
{
  ...
  //////// If sample has no affinity, then it can be ran in any worker in the queue
  if (isDefined(sample->_js.getJson(), "Worker Affinity") == false)
  {
  ...
  else //////// Else wait for the specific worker to be ready
  {
   workerId = KORALI_GET(size_t, (*sample), "Worker Affinity");

   // Checking if specified worker is available
   auto dequeFindItr = std::find(engine->_conduit->_workerQueue.begin(), engine->_conduit->_workerQueue.end(), workerId);

   while (dequeFindItr == engine->_conduit->_workerQueue.end())
   {
     //  If none are available, set sample's state back to initialized
     sample->_state = SampleState::initialized;
     ...
     // Re-checking for availability
     dequeFindItr = std::find(engine->_conduit->_workerQueue.begin(), engine->_conduit->_workerQueue.end(), workerId);
   }

   // Removing worker from queue
   engine->_conduit->_workerQueue.erase(dequeFindItr);
  }
