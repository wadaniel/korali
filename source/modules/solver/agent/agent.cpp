...
#pragma omp parallel for
      ...
      if (_optimizeMaxEntropyObjective == true)
      {
        ...
        {
          demonstrationTrajectoryLogProbabilities[n][i] = _demonstrationTrajectoryLogProbabilities[demIdx][0][0]; // TODO iterate agents
        ...
        for (size_t m = 0; m < _backgroundBatchSize; ++m)
        {
          ...
          demonstrationTrajectoryLogProbabilities[n][_demonstrationBatchSize + m] = _demonstrationTrajectoryLogProbabilities[demIdx][bckIdx + 1][0]; // TODO iterate agents
...
#pragma omp parallel for
        ...
        for (size_t n = 0; n < _demonstrationBatchSize; ++n)
        {
          backgroundTrajectoryLogProbabilities[m][n] = _backgroundTrajectoryLogProbabilities[bckIdx][0][0]; // probability from observed policy (TODO: it agents)
        ...
        if (_useFusionDistribution)
          for (size_t i = 0; i < _backgroundBatchSize; ++i)
          {
            ...
            backgroundTrajectoryLogProbabilities[m][_demonstrationBatchSize + i] = _backgroundTrajectoryLogProbabilities[bckIdx][bckIdx2 + 1][0]; // TODO: it agents
  ...
  for (size_t i = 0; i < retraceMiniBatch.size(); i++)
  {
    ...
    for (ssize_t curId = endId; curId >= startId; curId--)
    {
      ...
      if (_rewardUpdateBuffer[curId] < _rewardUpdateCount)
      {
        ...
        featureBatch[t++] = {_featureBuffer[curId][a]}; //TODO: it agents
