...
#pragma omp parallel for
      for (size_t k = 0; k < _maxEntropyGradient.size(); ++k)
      {
        // Contribution from partition function and demonstration batch
        if(_optimizeMaxEntropyObjective == true)
...
void Agent::deserializeExperienceReplay()
{
  ...
  for (size_t i = 0; i < stateJson["Experience Replay"].size(); i++)
  {
    ...
    _featureBuffer.add(stateJson["Experience Replay"][i]["Feature"].get<std::vector<float>>());
...
void Agent::setConfiguration(knlohmann::json& js) 
{
 ...
 {
 try { _featureRescalingMeans = js["Feature Rescaling"]["Means"].get<std::vector<float>>();
} catch (const std::exception& e)
 ...
 {
 try { _featureRescalingSigmas = js["Feature Rescaling"]["Sigmas"].get<std::vector<float>>();
} catch (const std::exception& e)
