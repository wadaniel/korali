...
namespace
{
 ...
 TEST(optimizers, LMCMAES)
 {
  ...
  // Creating optimizer configuration Json
  knlohmann::json optimizerJs;
  optimizerJs["Type"] = "Optimizer/LMCMAES";
  optimizerJs["Population Size"] = 32;

  // Creating module
  LMCMAES* opt;
  ASSERT_NO_THROW(opt = dynamic_cast<LMCMAES *>(Module::getModule(optimizerJs, &e)));
  ...
  // Setting up optimizer with failure
  optimizerJs = baseOptJs;
  experimentJs = baseExpJs;
  optimizerJs.erase("Population Size");
  ASSERT_ANY_THROW(opt->setConfiguration(optimizerJs));

  // Setting up optimizer with success
  optimizerJs = baseOptJs;
  experimentJs = baseExpJs;
  ASSERT_NO_THROW(opt->setConfiguration(optimizerJs));

  // Testing initial configuration fail
  opt->_targetDistanceCoefficients = std::vector<double>({0.0, 0.0});
  ASSERT_ANY_THROW(opt->setInitialConfiguration());

  // Testing initial configuration Success
  opt->_targetDistanceCoefficients = std::vector<double>({0.0, 0.0, 0.0});
  ASSERT_NO_THROW(opt->setInitialConfiguration());

  // Testing initial configuration Success
  opt->_targetDistanceCoefficients = std::vector<double>({});
  ASSERT_NO_THROW(opt->setInitialConfiguration());
  ...
  optimizerJs = baseOptJs;
  experimentJs = baseExpJs;
  optimizerJs["Initial Sigma"] = 1.0;
  ASSERT_NO_THROW(opt->setConfiguration(optimizerJs));

  optimizerJs = baseOptJs;
  experimentJs = baseExpJs;
  optimizerJs["Initial Sigma"] = std::vector<double>({1.0});
  ASSERT_ANY_THROW(opt->setConfiguration(optimizerJs));

  optimizerJs = baseOptJs;
  experimentJs = baseExpJs;
  optimizerJs.erase("Initial Sigma");
  ASSERT_ANY_THROW(opt->setConfiguration(optimizerJs));
  ...
  optimizerJs = baseOptJs;
  experimentJs = baseExpJs;
  optimizerJs.erase("Cumulative Covariance");
  ASSERT_ANY_THROW(opt->setConfiguration(optimizerJs));
  ...
  optimizerJs = baseOptJs;
  experimentJs = baseExpJs;
  optimizerJs["Termination Criteria"]["Min Value"] = std::vector<double>({1.0});
  ASSERT_ANY_THROW(opt->setConfiguration(optimizerJs));

  // Testing termination criteria
  e._currentGeneration = 2;
  opt->_maxGenerations = 10;
  opt->_minValue = -std::numeric_limits<double>::infinity();

  opt->_minValue = 2.0;
  opt->_bestEverValue = -3.0;
  ASSERT_FALSE(opt->checkTermination());
  opt->_bestEverValue = 3.0;
  ASSERT_TRUE(opt->checkTermination());
  opt->_minValue = -std::numeric_limits<double>::infinity();
