...
std::vector<float> Continuous::evaluateTrajectoryLogProbability(const std::vector<std::vector<std::vector<float>>> &states, const std::vector<std::vector<std::vector<float>>> &actions, const std::vector<float> &policyHyperparameter)
{
  knlohmann::json policyHyperParam;
      ...
      if (_policyDistribution == "Normal")
        for (size_t a = 0; a < _problem->_agentsPerEnvironment; ++a)
          for (size_t d = 0; d < _problem->_actionVectorSize; ++d)
            trajectoryLogProbability[a] += normalLogDensity(actions[t + b][a][d], policy[b * numAgents + a].distributionParameters[d], policy[b * numAgents + a].distributionParameters[_problem->_actionVectorSize + d]);
      else if (_policyDistribution == "Clipped Normal")
        for (size_t a = 0; a < _problem->_agentsPerEnvironment; ++a)
          for (size_t d = 0; d < _problem->_actionVectorSize; ++d)
            trajectoryLogProbability[a] += clippedNormalLogDensity(actions[t + b][a][d], policy[b * numAgents + a].distributionParameters[d], policy[b * numAgents + a].distributionParameters[_problem->_actionVectorSize + d], _actionLowerBounds[d], _actionUpperBounds[d]);
      else
...
std::vector<float> Continuous::evaluateTrajectoryLogProbabilityWithObservedPolicy(const std::vector<std::vector<std::vector<float>>> &states, const std::vector<std::vector<std::vector<float>>> &actions)
{
  std::vector<float> trajectoryLogProbability(_problem->_agentsPerEnvironment, 0.0);
